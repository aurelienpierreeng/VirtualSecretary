...
class Classifier:
    def __init__(self, feature_list: list[Data], embedding_list: list[str], path: str, validate: bool = True, variant: str = "svm", features: int = 300):
        ...
        if embedding_list:
            ...
            if os.path.exists(path + ".embed"):
                ...
                self.word2vec: gensim.models.KeyedVectors = gensim.models.KeyedVectors.load_word2vec_format(
    ...
    def get_features(self, post: str, external_data: list = [], language='english') -> dict:
        ...
        for word in nltk.word_tokenize(post.lower(), language=language):
            self.update_dict(features, self.wordvec(
                self.normalize_token(word, language)))
            i += 1
    ...
    def train_word2vec(self, embedding: list[str], language="english"):
        ...
        """
        sentences = [[self.normalize_token(token, language) for token in nltk.word_tokenize(
            sentence.lower())] for sentence in embedding]
    ...
    def train_model(self, validate: bool = False):
        ...
        new_featureset = [(self.get_features(post.text), post.label)
                          for post in self.set]
        ...
        if self.variant == "linear svm":
            ...
            classifier = RandomForestClassifier(
                n_jobs=-1, n_estimators=self.features)
        else:
            raise ValueError("Invalid classifier")

        self.model = SklearnClassifier(
            classifier, sparse=False).train(train_set)

        if validate:
            print("accuracy against train set:",
                  nltk.classify.accuracy(self.model, train_set))
            print("accuracy against test set:",
                  nltk.classify.accuracy(self.model, test_set))
    ...
    def classify_many(self, posts: list[str], external_data: list[str] = [], language='english') -> str:
        ...
        items = [self.get_features(post, external_data, language)
                 for post in posts]
    ...
    def prob_classify_many(self, posts: list[str], external_data: list[str] = [], language='english') -> list[tuple[str, float]]:
        ...
        output = [dict([(i, distro.prob(i)) for i in distro.samples()])
                  for distro in proba_distros]
